...
export namespace Helper {
    ...
    export function checkNetwork(confirm?: Function, cancel?: Function) {
        PluginMgr.checkNetwork()
            ...
            .catch(() => {
                ...
                UIMgr.OpenUI(cc.assetManager.getBundle("hzxlStart"), "component/loginTip/LoginTip", { param: param })
            })
    }
    ...
    export function checkBroke(needGold: number, callback: Function){        
        ...
        if(info && info.param.broke_num >= needGold && info.receive_num < info.day_times){
            UIMgr.OpenUI("component/Activity/Broke", { single: true, param: { activityConfig: info, callback: callback } })
        }
    }
        ...
        if(cc.sys.WECHAT_GAME === cc.sys.platform){
            if(cc.sys.os == cc.sys.OS_IOS){
                if(DataMgr.data.OnlineParam.wechat_audit == Constants.auditCode){
                    return true
                }else if(DataMgr.data.OnlineParam.shop_rounds && DataMgr.data.OnlineParam.shop_rounds > User.PlayGame){
                    return true
                }
            }
        }
    ...
    export function exchangeTemplateInfo(exchangeInfo: any, callback: Function){
        if (Helper.isAudit() == false && exchangeInfo.consume_list[0].item_id == Constants.ITEM_INDEX.GAME_DIAMOND && User.GameDiamond < exchangeInfo.consume_list[0].item_num) {	
            ...
            if(curBox){     
                ...
                let param = {
                    msg: "您的钻石不足，是否花费【¥" + curBox.price/100 + "】充值" + curBox.items[Constants.ITEM_INDEX.GAME_DIAMOND].num + "钻石？充值后将为您自动购买【" /*+ Helper.FormatNumWYCN(exchangeInfo.output_list[0].item_num)*/ + exchangeInfo.output_list[0].item_name +"】",
                    hideClose: true,
                    hideCancel: false,
                    confirm: () => {
