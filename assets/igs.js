...
var BasePlatform = (function () {
    ...
    BasePlatform.prototype.launchParam = function () { $warn("launchParam no implement"); };
    return BasePlatform;
}());
...
var WXPlatform = (function (_super) {
    ...
    WXPlatform.prototype.share = function (param) {
        ...
        if (param === void 0) { param = {}; }
        var self = this;
        var style = param.style || igs.platform.EShareStyle.Common;
        var queryExtStr = "";
        ...
        if (param.shareObj) {
            for (var i in param.shareObj) {
                if (param.shareObj[i]) {
                    if (i === "shareContent") {
                        paramCombine.shareContent = param.shareObj[i];
                    }
                    if (i === "sharePicUrl") {
                        paramCombine.sharePicUrl = param.shareObj[i];
                    }
        ...
        var shareCommon = function () {
            window['wx'].shareAppMessage({
                title: paramCombine.shareContent,
                imageUrl: paramCombine.sharePicUrl,
                query: queryStr,
                success: function (e) {
                    onSucc(e);
                },
            ...
            if (canvas) {
                ...
                canvas.toTempFilePath({
                    ...
                    success: function (obj) {
                        $log("save to file path success:", JSON.stringify(obj));
    ...
    WXPlatform.prototype.launchParam = function () {
        return this.launchP;
    };
    return WXPlatform;
}(BasePlatform));
...
(function (igs) {
    ...
    (function (odc) {
        ...
        var onTouchStart = function (t) {
            var pos = odcNode.node.convertTouchToNodeSpaceAR(t);
            wx.postMessage({
                id: _tag,
                message: "touch-start",
                mouseX: pos.x,
                mouseY: odcNode.node.height / 2 - pos.y
            });
        };
        ...
        odc.init = function (node) {
            do {
                ...
                if (odcNode) {
                    odcNode.node.removeComponent(cc.SubContextView);
